...
ncol(dem_score) # 10 variables
nrow(dem_score) # 96 observations
dim(dem_score) #row, column
head(dem_score, 15)
tail(dem_score)
str(dem_score)
```
**Question**
What does the second column of the `str` function show you? What are the possible different data types we might work with (review from PLCY 460?)? displays the internal structure of an object. The data can be quantitative or qualitative
...
dem_score[24,5] #what is the value in row 24, column 5: -7
...
what value is in the row United States in the column 1972
